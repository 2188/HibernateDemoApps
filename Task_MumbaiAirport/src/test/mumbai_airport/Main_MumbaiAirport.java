...
//		updateFlightData();		//********** Update flight data
		
		getFlightDetailsById(201100);		//********** Get flight data by Id
...
//		carrier.setFlightInfo(flightArray);
	...
	//********** Update flight data **********
		private static void updateFlightData() {
			...
			Carriers carrier = new Carriers();
			carrier.setCarrierID(1001);
			carrier.setCarrierName("Delta AirLines");
			...
			Location location = new Location();
			location.setLocationId(101);
			location.setLocationCode("101_PAR");
			location.setLocation_origin("Paris");
			...
			flightDetails_Obj.setFlight(flight);
			flightDetails_Obj.setLocation(location);
			flightDetails_Obj.setTravelTypeId(2);
			flightDetails_Obj.setStatusId(2);
			flightDetails_Obj.setArrivalTime("09:00 AM");
			flightDetails_Obj.setDepartureTime("12:30 PM");
			
			session.update(flightDetails_Obj);
		...
		private static void getFlightDetailsById(int id) {
			
			Session session = s_factory.openSession();
			
			FlightDetails  flightDetailObj = (FlightDetails) session.load(FlightDetails.class, id);
			
			TravelType  travelTypeObj = (TravelType) session.load(TravelType.class, flightDetailObj.getTravelTypeId());
			
			FlightStatus  flightStatus = (FlightStatus) session.load(FlightStatus.class, flightDetailObj.getStatusId());
			
			System.out.println("\n\n  Flight Name - "+flightDetailObj.getFlight().getFlightName()+
								"\n  Origin - "+flightDetailObj.getLocation().getLocation_origin()+
								"\n  Travel type - "+travelTypeObj.getTravelTypeName()+
								"\n  Flight Status - "+flightStatus.getFlight_Status()+
								"\n  Arrival time - "+flightDetailObj.getArrivalTime()+
								"\n  Departure time - "+flightDetailObj.getDepartureTime());
